...
class SentPairClassificationModel:
    ...
    def train_model(self, train_df, multi_label=False, output_dir=None, show_running_loss=True, args=None, eval_df=None, **kwargs):
        ...
        if 'text_a' in train_df.columns and 'text_b' in train_df.columns and 'labels' in train_df.columns:
            ...
            train_examples = [InputExample(i, text_a, text_b, label) for i, (text_a, text_b, label) in
                              enumerate(zip(train_df.iloc[:, 0], train_df.iloc[:, 1], train_df.iloc[:, 2]))]
    ...
    def evaluate(self, eval_df, output_dir, multi_label=False, prefix="", **kwargs):
        ...
        if 'text_a' in eval_df.columns and 'text_b' in eval_df.columns and 'labels' in eval_df.columns:
            ...
            eval_examples = [InputExample(i, text_a, text_b, label) for i, (text_a, text_b, label) in
                             enumerate(zip(eval_df.iloc[:, 0], eval_df.iloc[:, 1], eval_df.iloc[:, 2]))]
    ...
    def predict(self, to_predict, multi_label=False):
        ...
        if multi_label:
            ...
            eval_examples = [InputExample(i, text_a, text_b, None, [0 for i in range(self.num_labels)])
                             for i, (text_a, text_b) in enumerate(to_predict)]
        else:
            eval_examples = [InputExample(i, text_a, text_b, 0) for i, (text_a, text_b) in enumerate(to_predict)]
        if args['sliding_window']:
