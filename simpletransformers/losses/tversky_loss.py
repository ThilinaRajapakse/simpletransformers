...
class TverskyLoss(nn.Module):
    ...
    def forward(
        ...
        if len(input.shape) == 2:
            ...
            raise ValueError("Invalid input shape, we expect or NxC. Got: {}"
                             .format(input.shape))
        ...
        fps = torch.sum(input_soft * (1. - target_one_hot), -1)
        fns = torch.sum((1. - input_soft) * target_one_hot, -1)
