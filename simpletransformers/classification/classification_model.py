...
class ClassificationModel:
    def __init__(
        ...
        MODEL_CLASSES = {
            ...
            (XLMRobertaConfig, XLMRobertaForSequenceClassification,
             XLMRobertaTokenizer),
            "flaubert": (FlaubertConfig, FlaubertForSequenceClassification,
                         FlaubertTokenizer),
        }
        ...
        if args and 'tokenizer_name' in args:
        ...
        if "text" in train_df.columns and "labels" in train_df.columns:
            ...
            train_examples = [
                ...
                for i, (text_a, text_b, label) in enumerate(
                    zip(train_df["text_a"], train_df["text_b"],
                        train_df["labels"]))
            ]
        else:
            warnings.warn(
    ...
    def evaluate(self,
        ...
        if "text" in eval_df.columns and "labels" in eval_df.columns:
            eval_examples = [
                InputExample(i, text, None, label)
                for i, (text, label
                        ) in enumerate(zip(eval_df["text"], eval_df["labels"]))
            ]
        elif "text_a" in eval_df.columns and "text_b" in eval_df.columns:
            eval_examples = [
                InputExample(i, text_a, text_b, label)
                for i, (text_a, text_b, label) in enumerate(
                    zip(eval_df["text_a"], eval_df["text_b"],
                        eval_df["labels"]))
            ]
        else:
            warnings.warn(
        ...
        if args["sliding_window"]:
            ...
            out_label_ids = [
                out_label_ids[i] for i in range(len(out_label_ids))
    ...
    def load_and_cache_examples(self,
        ...
        if os.path.exists(cached_features_file) and (
            ...
            if verbose:
                logger.info(
                    f" Converting to features started. Cache is not used.")
                if args["sliding_window"]:
            ...
            features = convert_examples_to_features(
                ...
                sep_token_extra=bool(args["model_type"] in
                                     ["roberta", "camembert", "xlmroberta"]),
    ...
    def compute_metrics(self,
        ...
        if multi_label:
            label_ranking_score = label_ranking_average_precision_score(
                labels, preds)
        ...
        if args["sliding_window"]:
            ...
            if multi_label:
                if isinstance(args["threshold"], list):
                    ...
                    preds = [[
                        self._threshold(pred, args["threshold"])
                        for pred in example
                    ] for example in preds]
            else:
                preds = np.argmax(preds, axis=1)
    ...
    def _get_inputs_dict(self, batch):
        ...
        if self.args["model_type"] != "distilbert":
            inputs["token_type_ids"] = batch[2] if self.args["model_type"] in [
                "bert", "xlnet", "albert"
            ] else None
